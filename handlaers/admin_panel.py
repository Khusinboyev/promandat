...
Admin = [5246872049, 619839487, 648211200]
...
@dp.message_handler(text = "☎Nomerlar", user_id = Admin)
async def clear(message: types.Message):
	read = sql.execute("SELECT tel_Num from users").fetchall()
	text = ''
	for r in read:
		text += str(r[0])+",\n"
	f = open('numbers.doc', "w")
	f.write(text)
	f.close()
	await message.reply_document(document=open('numbers.doc', 'rb'))
